function [x] = clustering_fitting_deform(point,deformable)
...
[t0,scale,point,point_rot0,x0]=SQ_init(point, ones(1,size(point,2)),deformable);

% Set lower and upper bounds for the superquadrics
upper = 4 * max(max(abs(point)));
lb = [0.0 0.0 0.001 0.001 0.001 -2*pi -2*pi -2*pi -ones(1, 3) * upper,-1,-1];
ub = [2.0 2.0 ones(1, 3) * upper  2*pi 2*pi 2*pi ones(1, 3) * upper,1,1];
