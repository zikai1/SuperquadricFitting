function [point] = sphericalProduct_sampling_deform(para, arclength,tapering,bending)
...
if tapering == true
    fx=para(12).*point(:,3)/para(5)+1;
    fy=para(13).*point(:,3)/para(5)+1;
    fz=1;
...
if bending==true
    x_temp=point(:,1);
    y_temp=point(:,2);
    z_norm=sqrt(x_temp.^2+y_temp.^2);

    beta=atan(y_temp./x_temp);
    r=cos(para(13)-beta).*z_norm;

    gamma=point(:,3)/para(12);

    Radius=1/para(12)-cos(gamma)*(1/para(12)-r);


    point(:,1) = point(:,1)+cos(para(13))*(Radius-r);
    point(:,2) = point(:,2)+sin(para(13))*(Radius-r);
    point(:,3) = sin(gamma)*(1/para(12)-r);
