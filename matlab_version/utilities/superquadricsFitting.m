function [x] = superquadricsFitting(point, cost_function_type,deformable)
...
% -------------------------------------------------------------------------
options = optimoptions('lsqnonlin', 'Algorithm', 'levenberg-marquardt', 'Display','off', 'MaxIterations', 150);
% 'StepTolerance', 1e-10
...
if strcmp(cost_function_type,'Vanilla_IO')
    cost_func = @(x) vanilla_in_out_func(x, point,deformable);
end

if strcmp(cost_function_type, 'Power_IO')
    cost_func = @(x) power_in_out_func(x, point,deformable);
end

if strcmp(cost_function_type, 'Vol_Power_IO')
    cost_func = @(x) volume_power_in_out_func(x, point,deformable);
end

if strcmp(cost_function_type, 'Radial')
    cost_func = @(x) radial_distance_func(x, point,deformable);
end
...
% -------------------------------------------------------------------------
    function [value] = vanilla_in_out_func(para,X,deformable)
        ...
        if deformable
        % Inverse tampering transformation
            a3=para(5);
            kx=para(12);
            ky=para(13);

            xc = xc ./ (kx/a3 * zc + 1);% inverse tapering for canonical superquadrics
            yc = yc ./ (ky/a3 * zc + 1);
        end
    ...
    function [value] = power_in_out_func(para, X,deformable)
    ...
    function [value] = volume_power_in_out_func(para, X,deformable)
    ...
    function [value] = radial_distance_func(para, X,deformable)
        % Gross and Boult, 1988
...
% function [theta, phi] = point2angle(x, y, z)
