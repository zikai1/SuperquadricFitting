function x = numerical_fitting(point,deformable)
...
%% cost function
    ...
    function [C,J] = myFun(para, X, alpha_phi, alpha_psi, beta_psi, alpha_l, alpha_h,deformable)
        e1 = para(1);
        e2 = para(2);
        a1 = para(3);
        a2 = para(4);
...
options = optimoptions('lsqnonlin', 'Algorithm', 'levenberg-marquardt', 'Display','off', 'SpecifyObjectiveGradient', true,...
                       'MaxIterations', 150);%50 in default
...
cost_func = @(x) myFun(x, point, alpha_phi, alpha_psi, beta_psi, alpha_l, alpha_h,deformable);
